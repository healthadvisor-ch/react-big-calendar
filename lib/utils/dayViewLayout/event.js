...
function positionFromDate(date, min, total) {
  ...
  var fromBegin = _dates.default.merge(min, date)

  var diff = _dates.default.diff(min, fromBegin, 'minutes')
...
function normalizeDates(startDate, endDate, _ref) {
  ...
  if (+c === +s && c < e) {
    return [startDate, _dates.default.endOf(startDate, 'day')]
  } // Current day is in between start and end dates,
  // so we make it span all day.

  if (c > s && c < e) {
    return [current, _dates.default.endOf(current, 'day')]
