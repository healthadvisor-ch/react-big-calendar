...
function inRange(e, start, end, accessors) {
  ...
  var startsBeforeEnd = (0, _dates.lte)(eStart, end, 'day') // when the event is zero duration we need to handle a bit differently

  var endsAfterStart = (0, _dates.neq)(eStart, eEnd, 'minutes')
    ? (0, _dates.gt)(eEnd, start, 'minutes')
    : (0, _dates.gte)(eEnd, start, 'minutes')
...
function sortEvents(evtA, evtB, accessors) {
  var startSort =
    +(0, _dates.startOf)(accessors.start(evtA), 'day') -
    +(0, _dates.startOf)(accessors.start(evtB), 'day')
  var durA = (0, _dates.diff)(
    accessors.start(evtA),
    (0, _dates.ceil)(accessors.end(evtA), 'day'),
    'day'
  )
  var durB = (0, _dates.diff)(
    accessors.start(evtB),
    (0, _dates.ceil)(accessors.end(evtB), 'day'),
    'day'
  )
  return (
