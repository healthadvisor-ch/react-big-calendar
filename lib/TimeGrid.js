...
var TimeGrid = /*#__PURE__*/function (_Component) {
  ...
  _proto.positionTimeIndicator = function positionTimeIndicator() {
    var _this$props = this.props,
        ...
        useDynamicWidthOfTimeIndicator = _this$props.useDynamicWidthOfTimeIndicator,
    ...
    if (timeGutter && current >= min && current <= max) {
      ...
      if (useDynamicWidthOfTimeIndicator) {
        timeIndicator.style.width = timeGutter.parentElement.scrollWidth - timeGutter.offsetWidth + 'px';
      } else {
  ...
  _proto.renderDayColumn = function renderDayColumn(events, date, dateIndex, allResources, resource, resourceId, resourceIndex, now) {
    ...
    var daysEvents = (groupedEvents.get(resourceId) || []).filter(function (event) {
      return localizer.inRange(date, accessors.start(event), accessors.end(event), 'day');
    });
    return /*#__PURE__*/_react.default.createElement(_DayColumn.default, (0, _extends2.default)({}, this.props, {
      localizer: localizer,
      ...
      date: date,
      events: daysEvents,
      className: resourceIndex === allResources.length - 1 && resourcesGroupBy === 'date' ? 'rbc-time-column-last-in-resource' : undefined
    }));
  };

  _proto.renderEvents = function renderEvents(range, events, now, allResources) {
    var _this5 = this;
      ...
      return range.map(function (date, dateIndex) {
        return allResources.map(function (_ref, resourceIndex) {
          var resourceId = _ref[0],
              resource = _ref[1];
          return _this5.renderDayColumn(events, date, dateIndex, allResources, resource, resourceId, resourceIndex, now);
        });
      });
    }
  ...
  _proto.render = function render() {
    ...
    events.forEach(function (event) {
      ...
      if (accessors.allDay(event) || (0, _dates.isJustDate)(eStart) && (0, _dates.isJustDate)(eEnd) || !showMultiDayTimes && localizer.neq(eStart, eEnd, 'day')) {
    ...
    }), this.renderEvents(range, rangeEvents, getNow(), _resources), /*#__PURE__*/_react.default.createElement("div", {
      ref: "timeIndicator",
