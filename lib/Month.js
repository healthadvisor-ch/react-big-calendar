...
var eventsForWeek = function eventsForWeek(evts, start, end, accessors, localizer) {
  return evts.filter(function (event) {
    return localizer.inRange(accessors.start(event), localizer.startOf(start, 'day'), localizer.endOf(end, 'day'), 'week') || localizer.inRange(accessors.end(event), localizer.startOf(start, 'day'), localizer.endOf(end, 'day'), 'week');
  });
};
...
var MonthView = /*#__PURE__*/function (_React$Component) {
  ...
  function MonthView() {
    ...
    _this.renderWeek = function (week, weekIdx) {
      ...
      var key = week[0].toISOString().slice(0, 10) + "-" + weekIdx;
    ...
    _this.readerDateHeading = function (_ref) {
      ...
      var isCurrent = localizer.eq(date, currentDate, 'day');
