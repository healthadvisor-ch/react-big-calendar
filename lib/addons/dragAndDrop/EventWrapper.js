...
var EventWrapper = /*#__PURE__*/ (function(_React$Component) {
  ...
  function EventWrapper() {
    ...
    _this.handleStartDragging = function(e) {
      ...
      if (!isResizeHandle) _this.handleBeginAction(e, 'move')
    }

    _this.handleBeginAction = function(e, action, direction) {
      console.log(
        'EW - handleBeginAction - action ' + action + ' dir ' + direction
      )
      var lastNativeEvent =
        _this.context.draggable.dragAndDropAction.nativeEvent
      var nativeEvent = e.nativeEvent.type
      var touchEndThenMouseDown =
      ...
      console.log(
        'EW - nativeEvent: ' +
          ...
          interacting +
          ' touchEndThen ' +
          touchEndThenMouseDown
      ) // TODO is this still needed ?????

      if (
        (nativeEvent === 'touchend' && !interacting) ||
        touchEndThenMouseDown
      ) {
