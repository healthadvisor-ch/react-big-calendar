...
var WeekWrapper = /*#__PURE__*/ (function(_React$Component) {
  ...
  function WeekWrapper() {
    ...
    _this.handleMove = function(point, bounds) {
      ...
      if (!event) return // Make sure to maintain the time of the start date while moving it to the new slot

      var start = _dates.default.merge(
        slotMetrics.getDateForSlot(
          (0, _selection.getSlotAtX)(bounds, point.x, false, slotMetrics.slots)
        ),
  ...
  _proto.handleResize = function handleResize(point, bounds) {
    ...
    var slot = (0, _selection.getSlotAtX)(bounds, point.x, false, metrics.slots)
    var date = metrics.getDateForSlot(slot)
    ...
    if (direction === 'RIGHT') {
      if (cursorInRow) {
        ...
        end = _dates.default.add(date, 1, 'day')
      } else if (
        _dates.default.inRange(start, metrics.first, metrics.last) ||
        (bounds.bottom < point.y && +metrics.first > +start)
      ) {
      ...
      if (cursorInRow) {
        ...
        (bounds.top > point.y && +metrics.last < +end)
      ) {
