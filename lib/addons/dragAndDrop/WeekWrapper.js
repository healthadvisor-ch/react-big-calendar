...
var eventTimes = function eventTimes(event, accessors, localizer) {
  var start = accessors.start(event);
  var end = accessors.end(event);
  var isZeroDuration = localizer.eq(start, end, 'minutes') && start.getMinutes() === 0; // make zero duration midnight events at least one day long
...
var WeekWrapper = /*#__PURE__*/function (_React$Component) {
  ...
  function WeekWrapper() {
    ...
    _this.handleMove = function (_ref, node) {
      ...
      var start = localizer.merge(metrics.getDateForSlot((0, _selection.getSlotAtX)(rowBox, x, false, metrics.slots)), accessors.start(event));
      var end = localizer.add(start, localizer.diff(accessors.start(event), accessors.end(event), 'minutes'), 'minutes');
    ...
    _this._selectable = function () {
      ...
      if (!node) return;
      var closestNode = node.closest('.rbc-month-row, .rbc-allday-cell');
      if (!closestNode) return;
      var container = closestNode.closest('.rbc-month-view, .rbc-time-view');
      ...
      selector.on('beforeSelect', function (point) {
        ...
        return action === 'move' || action === 'resize' && (!isAllDay || (0, _selection.pointInBox)((0, _Selection.getBoundsForNode)(closestNode), point));
      });
      selector.on('selecting', function (box) {
        var bounds = (0, _Selection.getBoundsForNode)(closestNode);
  ...
  _proto.handleResize = function handleResize(point, node) {
    ...
    var _eventTimes = eventTimes(event, accessors, localizer),
    ...
    if (direction === 'RIGHT') {
      if (cursorInRow) {
        ...
        end = localizer.add(metrics.getDateForSlot((0, _selection.getSlotAtX)(rowBox, point.x, false, metrics.slots)), 1, 'day');
      } else if (localizer.inRange(start, metrics.first, metrics.last) || rowBox.bottom < point.y && +metrics.first > +start) {
      ...
      } else if (localizer.inRange(end, metrics.first, metrics.last) || rowBox.top > point.y && +metrics.last < +end) {
