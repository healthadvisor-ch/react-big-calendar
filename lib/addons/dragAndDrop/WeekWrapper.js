...
var eventTimes = function eventTimes(event, accessors, localizer) {
  ...
  var isZeroDuration =
    localizer.eq(start, end, 'minutes') && start.getMinutes() === 0 // make zero duration midnight events at least one day long
          ...
          (action === 'resize' &&
            (!isAllDay ||
              (0, _selection.pointInBox)(
                (0, _Selection.getBoundsForNode)(closestNode),
                point
              )))
        )
      })
