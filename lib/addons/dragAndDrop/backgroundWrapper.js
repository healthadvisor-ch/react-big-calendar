...
function getEventTimes(start, end, dropDate, type, localizer) {
  ...
  var nextStart = type === 'dateCellWrapper' ? localizer.merge(dropDate, start) : dropDate;
  ...
  if (type === 'dayWrapper' && window.updateDropPositionBasedOnDragPoint) {
    ...
    if (dragPointDistanceFromTop, draggedElementHeight) {
      var diff = parseInt(dragPointDistanceFromTop * duration / draggedElementHeight, 10); // Round to given step duration (15 minutes by default)
...
var DraggableBackgroundWrapper = /*#__PURE__*/function (_React$Component) {
  ...
  _proto.render = function render() {
    ...
    return /*#__PURE__*/_react.default.createElement(BackgroundWrapper, null, connectDropTarget(resultingChildren));
  };
  ...
  var dropTarget = {
    drop: function drop(_, monitor, _ref) {
      ...
      if (monitor.getItemType() === 'event') {
        onEventDrop((0, _extends2.default)({
          event: event
