...
function eventTimes(event, accessors) {
  ...
  var isZeroDuration = (0, _dates.eq)(start, end, 'minutes') && (0, _dates.minutes)(start) === 0; // make zero duration midnight events at least one day long

  if (isZeroDuration) end = (0, _dates.add)(end, 1, 'day');
  var duration = (0, _dates.diff)(end, start, 'milliseconds');
  return {
    start: start,
    end: end,
    duration: duration
