  ...
  function EventContainerWrapper() {
    ...
    _this._selectable = function() {
      ...
      selector.on('selecting', function(box) {
        ...
        var dragAndDropAction = _this.context.draggable.dragAndDropAction
        console.log('ECW - on selecting ' + dragAndDropAction.action) // most likely changes below are unnecessary (set state and new consts)

        var isMoving = dragAndDropAction.action === 'move'
        var isResizing = dragAndDropAction.action === 'resize'
        ...
        if (isMoving) _this.handleMove(box, bounds)
        if (isResizing) _this.handleResize(box, bounds)
      })
      ...
      selector.on('select', function(point) {
        ...
        if (
          ...
          _this.state.isMoving ||
          _this.state.isResizing
        ) {
          _this.setState({
            isMoving: false,
            isResizing: false,
